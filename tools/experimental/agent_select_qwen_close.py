...
# 路径（生产数据只读，绝对路径）
# ----------------------------------------------------------------------------
DATE = "2026-09-17"          # 收盘所在交易日(数据结算日)
NEXT = "2026-09-18"          # 目标交易日(次日)
PREV = "2026-09-16"
SECTOR_ASOF = "2026-09-17"   # 收盘后板块 focus/daily
CLOSE_MODE = True            # 收盘口径:现价=当日收盘,忽略午盘快照
DATA = Path("/Users/yqg/Documents/projects/stock_analysis/data")     # 读:生产数据主仓,只读
...
ANALYSIS_PREV = DATA / "analysis" / SECTOR_ASOF   # 收盘口径:用当日收盘后板块数据
ROSTER_DIR = DATA / "sector_roster"
# 写:重定向到本 worktree(不落主仓、不合main)
_WT_ROOT = Path(__file__).resolve().parents[2]
OUT_DIR = _WT_ROOT / "data" / "analysis" / DATE
DOCS_DIR = _WT_ROOT / "docs" / "每日分析" / "选股"
    ...
    {"type": "function", "function": {
        ...
        "parameters": {"type": "object", "properties": {
            "selections": {"type": "array", "items": {"type": "object", "properties": {
                ...
                "风险": {"type": "string"},
                "卖出计划": {"type": "string", "description": "次日进场后的目标/止盈/触发卖出条件"}},
                "required": ["code", "入场方式", "档", "建议分", "理由", "风险", "卖出计划"]}}},
            "required": ["selections"]}}},
]

SYSTEM_PROMPT = f"""你是选股分析师 Agent。现在是 {DATE} 收盘后，你有工具可自主调用来研究候选股，请多轮推理后给出【收盘口径】次日（{NEXT}）选股（次日开盘/盘中择机进、当日或数日内择机卖出，目标绝对收益为正）。数据均为收盘已结算口径：get_metrics/get_kline 的现价与涨跌均为 {DATE} 收盘值（无盘中快照，get_intraday 收盘口径下为空）。硬纪律：
...
USER_PROMPT = (f"现在是 {DATE} 收盘后，请做【收盘口径】次日（{NEXT}）选股，选出 5-8 只，"
               "次日开盘/盘中择机进、目标绝对收益为正。先看候选池，自主逐只研究最有希望的十几只"
               "（现价=今日收盘），再定稿并调用 submit_final。")
...
def _write_outputs(results, trace, mechanism, tool_calls, err, degraded):
    # 1) JSON
    with open(OUT_DIR / f"{DATE}_agent收盘_qwen.json", "w", encoding="utf-8") as f:
        json.dump({
    ...
    # 2) trace md
    lines = [f"# 千问3.8-max Agent 选股过程留痕 · {DATE} 收盘口径（次日 {NEXT}）", "",
    ...
    for step in trace:
        ...
        lines.append("")
    with open(OUT_DIR / f"{DATE}_agent收盘_qwen_trace.md", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))

    # 3) 操作卡 md（= 交付主文档）
    ol = [f"# 千问3.8-max Agent 选股 · 收盘口径操作卡 · {DATE}（次日 {NEXT}）", "",
          f"> 机制：{mechanism}｜模型：`{MODEL}`｜研究模拟、非投资建议、只研究不下单。收盘口径为次日选股。价位为程序按均线/前低回填。", ""]
    for r in results:
        ...
        ol.append("")
    card_text = "\n".join(ol)
    with open(OUT_DIR / f"{DATE}_agent收盘_qwen_操作卡.md", "w", encoding="utf-8") as f:
        f.write(card_text)
    # 交付主文档:docs/每日分析/选股/2026-09-17_agent收盘_qwen.md
    DOCS_DIR.mkdir(parents=True, exist_ok=True)
    with open(DOCS_DIR / f"{DATE}_agent收盘_qwen.md", "w", encoding="utf-8") as f:
        f.write(card_text)
