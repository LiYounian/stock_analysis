...
# ---------------------------------------------------------------------------
TODAY = "2026-09-17"          # 收盘所在交易日(数据结算日)
NEXT = "2026-09-18"           # 目标交易日(次日)
KLINE_ASOF = "2026-09-17"     # K线已结算至收盘
SECTOR_ASOF = "2026-09-17"    # 收盘后板块 focus/daily
CLOSE_MODE = True             # 收盘口径:现价=当日收盘,忽略午盘快照
MODEL_ID = os.environ.get("LLM_MODEL") or "deepseek-v4-pro"
MAX_ITERS = 40

# 读:生产数据在主仓(绝对路径,只读)；写:重定向到本 worktree(不落主仓、不合main)
DATA_ROOT = "/Users/yqg/Documents/projects/stock_analysis/data"
# tools/experimental/<this>.py → 上溯3层 = worktree 根
_WT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
OUT_DIR = os.path.join(_WT_ROOT, "data", "analysis", TODAY)
DOCS_DIR = os.path.join(_WT_ROOT, "docs", "每日分析", "选股")
...
if CLOSE_MODE:
    # 收盘口径:丢弃午盘冻结快照,现价/涨跌一律以已结算收盘K线(09-17)为准
    T1145, SNAP = {}, {}
...
def compute_metrics(code: str) -> dict:
    ...
    # 若无盘中价(收盘口径),用已结算收盘作现价(setdefault对已存的None无效,需显式覆盖)
    ref_price = float(price) if price else last_close
    if out.get("现价") is None:
        out["现价"] = ref_price
    ...
    pct = out.get("当日涨跌%")
    # 收盘口径:无盘中快照时,用已结算收盘K线推当日涨跌(收盘 vs 前一收盘)
    if pct is None and len(closes) >= 2:
        prev_c = float(closes.iloc[-2])
        if prev_c:
            pct = round((last_close - prev_c) / prev_c * 100, 2)
            out["当日涨跌%"] = pct
    if pct is not None:
...
# ---------------------------------------------------------------------------
SYSTEM_PROMPT = f"""你是选股分析师 Agent。现在是 {TODAY} 收盘后，你有工具可自主调用来研究候选股，请多轮推理后给出【收盘口径】次日（{NEXT}）选股（次日开盘/盘中择机进、当日或数日内择机卖出，目标绝对收益为正）。数据均为收盘已结算口径：get_metrics/get_kline 的现价与涨跌均为 {TODAY} 收盘值（无盘中快照）。硬纪律：
...
USER_PROMPT = f"""现在是 {TODAY} 收盘后，请做【收盘口径】次日（{NEXT}）选股，次日开盘/盘中择机进、目标绝对收益为正。
请从候选池中经充分研究后选出 5-8 只。研究流程建议：先 list_candidates 看全池，锁定十几只有希望的（优先数据面综合分高+消息面看多+来源可靠的），逐只用 get_metrics 看量化画像（现价=今日收盘）、必要时 get_kline 看形态、get_sector_context 看板块角色与催化，最后综合定档。
...
def main():
    ...
    if not base or not key:
        ...
        # DeepSeek 首 token 延迟高,timeout 放宽到 600s
        client = OpenAI(base_url=base, api_key=key, timeout=600.0, max_retries=0)
...
def write_outputs(results, trace, usage, mechanism, final_text):
    ...
    with open(os.path.join(OUT_DIR, f"{TODAY}_agent收盘_deepseek.json"), "w", encoding="utf-8") as fh:
    ...
    lines.append("```")
    with open(os.path.join(OUT_DIR, f"{TODAY}_agent收盘_deepseek_trace.md"), "w", encoding="utf-8") as fh:
        fh.write("\n".join(lines))

    # 9.3 操作卡
    cards = []
    cards.append(f"# DeepSeek-Agent 选股 操作卡 · {TODAY} 收盘口径（次日 {NEXT}）")
    cards.append("")
    cards.append(f"> 机制：{mechanism}｜模型：`{MODEL_ID}`｜研究模拟非投资建议、只研究不下单｜收盘口径为次日选股")
    cards.append("")
    ...
    for r in sorted(results, key=lambda x: (order.get(x["档"], 9), -float(x.get("建议分", 0) or 0))):
        ...
        cards.append(f"- 卖出：次日择机卖出。{r['卖出计划']}")
        cards.append(
        ...
        cards.append("")
    card_text = "\n".join(cards)
    with open(os.path.join(OUT_DIR, f"{TODAY}_agent收盘_deepseek_操作卡.md"), "w", encoding="utf-8") as fh:
        fh.write(card_text)
    # 交付主文档:docs/每日分析/选股/2026-09-17_agent收盘_deepseek.md
    os.makedirs(DOCS_DIR, exist_ok=True)
    with open(os.path.join(DOCS_DIR, f"{TODAY}_agent收盘_deepseek.md"), "w", encoding="utf-8") as fh:
        fh.write(card_text)

    print(f"[out] 已写产物到 {OUT_DIR} 及 {DOCS_DIR}", file=sys.stderr)
