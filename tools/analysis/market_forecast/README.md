...
> 📇 **规范化模型卡**（统一 schema，组权重现值 + `文件:行` 出处 + 变更日志）：`docs/参考/模型卡/market_forecast.md`。
> 组权重真源 = `tools/config/strategy.py:370`；文档↔代码一致性由 `tests/test_model_card_weight_consistency.py` 锁死。
