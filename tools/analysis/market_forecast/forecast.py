...
⚠️ 效力现状(2026-09-10 维度贡献研究):标称四维,**每天真正在判别的只有技术+广度**
(资金流权重=0 kill-switch;消息面覆盖率≈1%被自动降权≈0);整体命中~55% 只是**方向微边际、
多空收益价差≈0(无经济 alpha)**。非投资建议——方向概率供参考,勿当能赚钱的信号。
"""
...
def build_forecast(as_of: str | None = None, targets=("hs300", "proxy"),
    ...
    return {
        ...
        "notes": ("档位=**上行概率分位(方向口径)**,不是涨跌幅预测;幅度未知。"
                  "⚠️ 效力诚实标注(2026-09-10 维度贡献研究,见 docs/计划/2026-09-10_市场情绪预测维度贡献研究.md):"
                  "整体方向命中~55%,较纯惯性有约 +4.8pp 的**统计边际(显著)**,但**多空收益价差≈0(无经济 alpha)**——"
                  "即方向能微弱猜、收益换不出来,一部分命中是顺着广度/动量的 β;**勿把高概率读成能赚钱**。"
                  "标称四维,但**每天真正参与判别的只有技术+广度两维**:资金流权重=0(kill-switch,2026-09-02),"
                  "消息面历史仅~1月→长样本覆盖率≈1%被自动降权到≈0(且从未经样本外验证)。"
                  "技术与广度高度冗余(去一维命中几乎不掉)。个股选股 β 基准默认 proxy(全A等权≈中小盘,含幸存者偏差,"
                  "绝对收益勿当真),hs300 仅权重股背景,分歧看'分歧标记'。"
                  "A/B回测详见 docs/计划/大盘预测策略.md §7 与 "
