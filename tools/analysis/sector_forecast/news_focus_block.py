...
sector_focus.json**(选股已读它、少一个源),顶层加 `消息驱动` 块:
  {as_of, 利好板块:[{board, tag, 强弱, 关键事件, 持续性, 时效, 可靠性综述, 依据,
                    龙头候选:[{code,name,已动}],          # 已动=当日涨幅≥MOVED_PCT(3%)
                    跟涨候选:[{code,name,联动依据}]}]}
注:龙头候选**不带 `催化` 字段**——该票的催化内容在**板块级 `关键事件`**(逐条带
时间/方向/可信度/影响程度/执行度/来源),按 board join 取,不重复到每个候选上。
