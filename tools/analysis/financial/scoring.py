...
def dimension_specs() -> dict:
    """五维各子指标的标准化区间(0分端 → 100分端),供详情页展示"审计标准"与 M3 标定。

    ⚠️ 值必须与 five_dims() 内联映射一致(此处是可读镜像;改标定时两处同步,或后续外化到 config)。
    每项:(显示名, derived 键名, 0分端值, 100分端值);反向指标 0分端 > 100分端。
    """
    return {
        "成长": [("营收增速%", "营收增速", -20, 40), ("扣非净利增速%", "扣非净利增速", -20, 40)],
        "质量": [("现金含量 CFO/归母净利", "现金含量_CFO比净利", 0, 1.2),
               ("扣非占归母", "扣非占归母", 0, 1.0), ("毛利率%", "毛利率", 0, 60)],
