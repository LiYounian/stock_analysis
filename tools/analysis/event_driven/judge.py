...
def classify_share_change(action_type: str, method: str | None = None,
    ...
    类别语义(优先级:战略引资 > 二级减持 > 非减持承诺 > 协议转让待定 > 普通减持):
      - 二级减持:大股东/高管经集中竞价/大宗在二级市场卖出 → 真看空(保留;真抛售优先级最高)。
      - 战略引资:协议转让给产业资本/战投/同业龙头、引入战投、业务协同 → 利好/中性(不看空)。
      - 非减持承诺:重组报告书"承诺不减持/锁定期/转让限制"条款文本 → **非真实减持**,不计利空(中性)。
    ...
    excluded = _has_kw(blob, "减持排除关键词")
    if strategic:                                    # 产业资本/战投受让:引资背书,非抛售
        return {"类别": "战略引资", "看空有效": False, "偏正": True,
                "依据": "协议转让给产业资本/战投(引资+背书,非二级抛售)"}
    if secondary:                                     # 集中竞价/大宗:二级市场套现(真抛售优先)
        return {"类别": "二级减持", "看空有效": True, "偏正": False,
                "依据": "二级市场集中竞价/大宗减持套现"}
    if excluded:                                      # 重组"承诺不减持/锁定/限售"条款:非真实减持
        return {"类别": "非减持承诺", "看空有效": False, "偏正": False,
                "依据": "重组承诺不减持/锁定期/转让限制(非二级抛售,不计利空)"}
    if protocol:                                      # 协议转让但受让方性质不明:识别不清
...
def judge_corporate_action(action_type: str, scale_ratio: float | None = None,
    ...
    if action_type == "减持":
        ...
        if 类别 in ("协议转让待定", "非减持承诺"):     # 识别不清/重组承诺 → 中性、不计利空
            return {"方向": "中性", "强度": 0.0, "象征性": False,
                    "依据": f"减持·{类别}·" + cls["依据"], "类别": 类别}
        # 二级减持 / 普通减持 → 落常规看空强度逻辑
    else:
