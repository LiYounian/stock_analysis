...
⚠️ 已下线·仅存档:S01 判定显著负已从生产链路摘除,本驱动仅供存档复现。详见 docs/计划/S01诊断_删除判定_20260829.md。
...
def _board(code: str) -> str:
    """回测分档标签。**不是**交易所判定(交易所真源见 `tools.config.exchange`)。

    ⚠️ **已知缺口(2026-09-03,有意未修)**:北交所只认 8/4 段,漏 920 现行段 → 920 票
    归"其他"。改了会让回测分档汇总数变化(口径变更),故本轮不动,另开任务拍板。
    """
