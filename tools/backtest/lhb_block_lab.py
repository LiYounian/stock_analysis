...
from tools.collectors import index, lhb

# block_trade 采集器为大宗交易(本 veto 专线不带,可选)。lhb-only 用法下不 import,
# 避免在只落 lhb.py 的分支上因缺 block_trade.py 而 ImportError(文件归属边界)。
try:                                     # pragma: no cover - 依落盘文件是否带 block_trade
    from tools.collectors import block_trade
except ImportError:                      # 大宗交易采集器不在本分支 → block 源不可用(优雅降级)
    block_trade = None
...
def fetch_events(source: str, start: str, end: str) -> pd.DataFrame:
    ...
    for s, e in _quarters(start, end):
        if source == "lhb":
            ...
            if block_trade is None:
                raise RuntimeError("block 事件源不可用:本分支未带 tools/collectors/block_trade.py")
