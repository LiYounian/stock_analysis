...
⚠️ 测试环境研究模拟，非投资建议。**forward-shadow 纯记录·non-gating·不进任何生产选股决策**
（已合 main 作每日 shadow runner，攒 ≥120 样本前不 gate；CLI 见文件末 `main()`）。
"""
...
# forward-shadow 统一归 data/shadow_forward/ 下（与既有 shadow_forward 约定同处、node 派生数据）。
DEFAULT_OUT_DIR = "data/shadow_forward/sector_inflection"
DEFAULT_SHADOW_DIR = "data/shadow_forward/sentiment"


def _load_all_klines(data_root: str | None = None, min_bars: int = 60) -> dict:
    """全A主档 K 线 {code: df(升序)}，date 列归一为 str 'YYYY-MM-DD'（select_members 按 str 比较）。
    复用 screen_forward_common 的已测加载器（排北交所/丢历史不足）。"""
    ...
    ensure_data_root(data_root)  # 缺省自动探测主仓 data/，接 store 读全A主档
    codes = sfc.universe_codes()
    klines = sfc.load_klines(codes, min_bars=min_bars)
...
def main(argv: list[str] | None = None) -> int:
    """每日 shadow runner：检测板块拐点 + 成员候选 → 落 advisory（non-gating）。
    退出码：0=成功/非交易日跳过/幂等跳过；非0=数据缺失（全A K线为空）。"""
    import argparse
    ...
    ap = argparse.ArgumentParser(
        description="④ 消息驱动板块拐点 forward-shadow runner（纯记录·non-gating）")
    ap.add_argument("--date", default=None, help="YYYY-MM-DD，缺省今天（非今天=历史补跑）")
    ap.add_argument("--out-dir", default=DEFAULT_OUT_DIR, help="advisory 落盘目录")
    ap.add_argument("--shadow-dir", default=DEFAULT_SHADOW_DIR,
                    help="情绪 shadow 目录（未积累时净A度返 {}、优雅缺省）")
    ap.add_argument("--data-root", default=None, help="缺省自动探测主仓 data/")
    ...
    klines = _load_all_klines(args.data_root)
    if not klines:
        logger.error("全A K线为空（数据根/主档问题），无法出 shadow")
        return 2
    ref_dates = sorted({d for df in klines.values() for d in df["date"]})

    adv = run_daily_shadow(date, args.out_dir, args.shadow_dir, klines, ref_dates)
    logger.info("shadow 落盘 %s：%d 板块拐点 → %s",
                date, len(adv["板块拐点"]), out_path)
    return 0
