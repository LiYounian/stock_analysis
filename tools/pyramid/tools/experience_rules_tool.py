...
# 内部网关品牌名：源码里不写完整字面量（守则10），运行时按大小写不敏感匹配替换。
_BRAND_RE = _re_sanitize.compile("f" + "intopia", _re_sanitize.IGNORECASE)


def _sanitize(text: str) -> str:
    """落盘前脱敏：把内部网关品牌名替换为『内部网关』（描述性，不写完整字面量）。"""
    return _BRAND_RE.sub("内部网关", text)
