...
# ── v2 影响模板（减持嫌疑档→对选股影响；共性"减持档怎么定"已进统一词表）──
_减持影响 = {
    "高": "套现抛压大+内部人看空、显著减分(veto候选)",
    "中": "减持抛压、减分",
    "低": "轻度减持、关注",
    "无嫌疑": "无减持抛压、治理面友好",
}
...
class InsiderReductionTool:
    ...
    def run(self, as_of: str, code: Optional[str] = None,
        ...
        if 档 == "无嫌疑":
            ...
            return ToolResult(
                name=self.name, 塔层=self.塔层, as_of=as_of, code=code,
                浓缩块=浓缩块([f"股东减持嫌疑: 无嫌疑（近{RECENT_DAYS}日无计入减持公告）影响：{_减持影响['无嫌疑']}", line2]),
        ...
        lines = [
            f"股东减持嫌疑: {档}（窗内计入 {n} 条·最近 {最近str}）影响：{_减持影响.get(档, '')}",
            f"方式: {方式}　主体: {主体}",
            f"样本: {主标题[:34]}",
        ]
        return ToolResult(
