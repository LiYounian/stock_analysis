...
数据源(实测):东财 `stock_news_em(symbol=<关键词>)`——按关键词返回结构化条目
(标题/内容/时间/来源/链接),与 news.py 同源可复用。pandas 3.0 默认 pyarrow
字符串会触发 akshare 内部正则报错,调用前关掉 `future.infer_string`(实测可绕过)。
备源 `news_cctv`(新闻联播,偏国内宏观)亦可用,当前未纳入,留作扩展口。
...
_POLICY_DIR = settings.DATA_RAW / "policy"
...
def _policy_path(date: str):
    return _POLICY_DIR / f"policy_{date}.json"
...
def tag_and_dump(raw_items: list[dict], days: int | None = None,
                 require_industry_hit: bool = True) -> list[dict]:
    """归一 + region/行业命中打标 + 时间窗过滤 + 去重 + 落盘。
    ...
    _policy_path(today).write_text(
        json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
    logger.info("政策 %s:入库 %d 条(去重后)", today, len(out))
    return out
...
def fetch_policy(keywords: list[str] | None = None, days: int | None = None) -> list[dict]:
    ...
    机制:逐关键词调东财 stock_news_em → 汇总 → 交 tag_and_dump 归一/打标/去重/落盘。
    单关键词失败记 logger 跳过,不中断整批;全部失败(拿到空)抛错不静默。
    """
    ...
    if not raw_items:
        raise RuntimeError(
            f"政策采集拉到空(全部 {len(keywords)} 个关键词无结果/失败),疑接口异常")
    return tag_and_dump(raw_items, days=days)
...
def load_policy(date: str | None = None) -> list[dict]:
    ...
    p = _policy_path(date)
    if not p.exists():
        raise FileNotFoundError(f"{date} 无政策缓存,请先 fetch_policy: {p}")
