...
2. 扩召回后**用 LLM(关思考)做相关性初筛**,**宁严**——只留与该股**直接相关**的
   (讲这只股票、或其主营/直接上下游的重大影响),滤掉泛大盘综述/无关个股噪声。
...
def _relevance_instruction(name: str, industry: str | None) -> str:
    """相关性初筛指令(**宁严**):只留讲本股 / 其主营 / 直接上下游重大影响的条目。"""
    ind = f"(行业:{industry})" if industry else ""
    return (
        f"你是股票新闻相关性初筛助手。判断下面这条新闻是否与股票【{name}】{ind}**直接相关**。"
        f"判定从严(宁可漏判也不错判):\n"
        f"- 只有当新闻**明确讲这只股票本身**,或讲它的**主营业务 / 直接上下游**并对其有"
        f"**重大影响**时,才算「相关」。\n"
        f"- 泛大盘/指数综述、宏观例行播报、与本股主营无直接关系的其他个股或子行业消息、"
        f"仅同板块泛泛提及 → 一律「不相关」。\n"
        f"- 拿不准时判「不相关」。\n"
        f"只输出「相关」或「不相关」。")
