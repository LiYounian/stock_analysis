...
> 版本:v1 · 2026-09-09 · 状态:**待用户在数据源窗回复**
...
- [ ] 采纳**方案 A**(数据线新建 `macro.py` + `kind="macro"`,归数据线)?
- [ ] 还是**方案 B**(REVS 自建,快但破分层)?
- [ ] 若 A:宏观 collector 是否**本轮就排期**,还是**等 REVS 阶段2 启动时再做**(阶段1 不阻塞)?

> 用户在本(数据源)窗口回复;定了归属后,A 的宏观 collector 详细设计另起文档。
