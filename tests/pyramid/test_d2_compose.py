...
def test_板块角色_龙头强RS高于无角色():
    龙头 = C.子分_板块角色({"角色": "龙头", "RS档": "强", "拥挤档": "B"})
    无 = C.子分_板块角色({"角色": None, "RS档": "弱", "focus_score": 0.5})
    assert 龙头 == 80 and 龙头 > 无


def test_板块角色_拥挤A降分():
    a = C.子分_板块角色({"角色": "中军", "RS档": "中", "拥挤档": "A"})
    b = C.子分_板块角色({"角色": "中军", "RS档": "中", "拥挤档": "B"})
    assert b - a == 10
