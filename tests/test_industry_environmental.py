"""环保行业财报专家单测(锁语义)。契约合法 + 高杠杆/慢回款区间校准 + 专属红旗边界 + 空输入不抛异常。"""
import importlib

mod = importlib.import_module("tools.analysis.financial.industry.环保")
...
def test_leverage_and_receivable_relaxed():
    ...
    assert dar[2] >= 80 and dar[2] > dar[3]
    ar = [s for s in specs["运营"] if s[1] == "应收周转天数"][0]
    assert ar[2] >= 250 and ar[2] > ar[3]
...
def test_flag_negative_cfo_hit_and_miss():
    hit = mod.extra_flags({}, {"利润表": {"归母净利润": 4e8},
                               "现金流量表": {"经营活动现金流量净额": -2e8}})
    assert "经营现金流为负" in _codes(hit)
    miss = mod.extra_flags({}, {"利润表": {"归母净利润": 4e8},
