...
def test_combined_union_and_sources(monkeypatch):
    ...
    assert strat["策略6"]["label"] == "自选池半导体多因子"
...
# ————————————————————————————————————————————————
# 策略6(自选池半导体多因子,web 层实时跑,不读 view)
# ————————————————————————————————————————————————
...
def test_strategy6_runs_in_web_layer(monkeypatch):
    """策略6 web 层实时跑:3 因子加权(rd_rev × 0.6 + rd_mcap × 0.2 + rev_yoy × 0.2)。"""
    recs = {
    ...
    page = da.selection_page()
    s6 = page["strategy6"]
    assert s6["present"] is True and s6["扫描数"] == 3 and s6["样本数"] == 3
...
def test_strategy6_missing_financial_derived(monkeypatch):
    """financial.derived 缺失 → 该票剔;剩余 <2 样本无法标准化 → present=False。"""
    recs = {
    ...
    # 仅剩 1 只可用 → 样本 <2 → 空 codes
    assert s6["picks"] == [] and s6["present"] is True   # present 依然 True(有 records 输入)


def test_strategy6_empty_pool(monkeypatch):
    """自选池无 records → present=False,不炸。"""
    _patch(monkeypatch, {})
    s6 = da.selection_page()["strategy6"]
