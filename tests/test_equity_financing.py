...
# ————————————————————————————————————————————————
# 8. 定增「筹划」首披不得漏计(漏计侧回归锁)
#
# 背景:一笔定增的**首次披露**往往是「关于**筹划**向特定对象发行股票的提示性公告」。它匹配
# 定增标题正则,但标题里**没有**预案/受理/问询等在途词——旧 LIVE 白名单不含「筹划」,于是
# plan_stage_signal 判 None → 这条首披被 normalize_seo_plan 丢弃,导致「最近推进中披露日」
# 错锚到更晚的预案日(实测首披 04-29 被丢、报成 06-27,事件锚点后移约 5 周)。
# 与「阶段误判(多计)」那轮是**同一维度的反向缺陷**:多计治了「条数冒充笔数」,这里补漏计。
# ————————————————————————————————————————————————
def test_筹划首披被判为进行信号():
    """① 「筹划…提示性公告」是定增在途首披 → 阶段信号必须为「进行」,不得被判 None 丢弃。"""
    t = "关于筹划向特定对象发行股票的提示性公告"
    assert efin.plan_stage_signal(t) == "进行", t
    rec = efin.normalize_seo_plan({"公告标题": t, "公告时间": "2026-04-29"})
    assert rec is not None and rec["阶段"] == "推进中" and rec["阶段信号"] == "进行"


def test_筹划首披纳入推进中且锚定最早披露日():
    """② 端到端:首披(筹划 04-29)+ 后续预案(06-27)是**同一笔**,推进中=1,
    且「最近推进中披露日」应锚到最早的首披、而不是把首披漏掉后错报成 06-27。"""
    rows = _plans(("关于筹划向特定对象发行股票的提示性公告", "2026-04-29"),
                  ("关于向特定对象发行股票预案的公告", "2026-06-27"))
    blk = _summarize_plans(rows, code="300802")
    assert blk["推进中"] == 1, blk["笔"]
    assert blk["推进中公告条数"] == 2                       # 两条都在,首披没被丢
    assert blk["笔"][0]["起始披露日"] == "2026-04-29"       # ← 锚点是首披,不是 06-27
    assert blk["最近推进中披露日"] == "2026-06-27"


def test_筹划被终止仍先判终止不误升为推进中():
    """别修过头:终止/已实施信号仍先于「筹划」判定(判定顺序不变),含筹划的终止公告仍判终止。"""
    assert efin.plan_stage_signal("关于终止筹划向特定对象发行股票事项的公告") == "终止"


# ————————————————————————————————————————————————
# 9. 解禁数量自洽性护栏(不自洽的解禁数据不静默采用)
#
# 背景:sina 报某批解禁数量远超「现存限售股」(实测某票 1453.2万股 vs 现存限售 447.19万股,3.25×,
# 物理不可能),而 em 无该批记录、cninfo 检索 0 行;期间发生过资本公积转增(除权前后口径可能串号)。
# 这类数量**不可采信**,不能直接拿去算摊薄比例。护栏:超过「现存限售股上限」(em 口径 解禁+未解禁
# 峰值 / 或绝对上界总股本)即标 `自洽性=不可采信`,消费侧不计入占比、只显式列出并提示交叉验证。
# ————————————————————————————————————————————————
def test_解禁数量超em口径限售股上限标不可采信():
    """① em 无该批次、但 em 口径现存限售股上限仅 447.19万股;sina 报 1453.2万股(3.25×)→ 不可采信。"""
    sina = [{"解禁日期": "2026-12-01", "解禁数量": 1453.2, "公告日期": "2026-04-29"}]
    em = [{"解禁时间": "2025-06-10", "实际解禁数量": 2_000_000, "未解禁数量": 2_471_942,
           "占流通市值比例": 1.0, "限售股类型": "首发原股东限售股份"}]
    rows, stat = efin.normalize_unlocks(sina, em)
    assert stat["em_限售股上限_股"] == pytest.approx(4_471_942)   # 200万 + 247.1942万
    r = [x for x in rows if x["解禁日"] == "2026-12-01"][0]
    assert r["增强源"] is None                                    # em 确无该批次
    assert r["自洽性"] == "不可采信", r
    assert "交叉验证" in r["自洽性依据"] and "3.25" in r["自洽性依据"]
    assert stat["不可采信"] == 1


def test_不可采信解禁不进摊薄测算且显式提示():
    """② 消费侧:不可采信的解禁**不算占比**(占总股本_pct=None、near_ratio 不含它),
    但仍在「下一次/明细」里显式列出并带 自洽性 标记 + 约束提示,不静默吞掉。"""
    sina = [{"解禁日期": "2026-12-01", "解禁数量": 1453.2, "公告日期": "2026-04-29"}]
    em = [{"解禁时间": "2025-06-10", "实际解禁数量": 2_000_000, "未解禁数量": 2_471_942}]
    rows, stat = efin.normalize_unlocks(sina, em)
    payload = {"code": "300802", "可转债": [], "定增": [], "解禁": rows,
               "解禁统计": stat, "源状态": {}, "降级": []}
    s = efin.summarize_asof(payload, as_of="2026-09-03", 总股本=1e8)
    blk = s["解禁"]
    assert blk["不可采信次数"] == 1
    nxt = blk["下一次"]
    assert nxt["解禁日"] == "2026-12-01" and nxt["自洽性"] == "不可采信"
    assert nxt["占总股本_pct"] is None                            # 不可采信 → 不折算摊薄
    assert blk[f"未来{efin.NEAR_UNLOCK_DAYS}日占流通_pct"] is None  # 占比不含不可采信条
    assert any("不可采信" in n for n in s["约束提示"]), s["约束提示"]


def test_解禁数量超总股本绝对上界标不可采信():
    """③ 无 em 上界时,总股本是绝对物理上界:单批解禁 5000万股 > 总股本 1000万股 → 不可采信。
    (归一阶段无总股本、无 em → 先判可采信;summarize_asof 拿到总股本后补核并升级。)"""
    sina = [{"解禁日期": "2026-12-01", "解禁数量": 5000.0, "公告日期": "2026-04-29"}]
    rows, stat = efin.normalize_unlocks(sina, [])
    assert rows[0]["自洽性"] == "可采信"                          # 归一阶段无上界 → 不猜
    payload = {"code": "x", "可转债": [], "定增": [], "解禁": rows,
               "解禁统计": stat, "源状态": {}, "降级": []}
    s = efin.summarize_asof(payload, as_of="2026-09-03", 总股本=1e7)
    nxt = s["解禁"]["下一次"]
    assert nxt["自洽性"] == "不可采信" and "总股本" in nxt["自洽性依据"]
    assert nxt["占总股本_pct"] is None


def test_自洽的解禁保持可采信不误伤(mocked):
    """④ 别修过头:正常解禁(数量远小于限售股池/总股本)必须保持可采信,占比照常折算。"""
    payload = efin.fetch_one("603270")
    s = efin.summarize_asof(payload, as_of=AS_OF, 总股本=SHARES)
    assert all(u.get("自洽性", "可采信") == "可采信" for u in s["解禁"]["明细"]), s["解禁"]["明细"]
    assert s["解禁"]["不可采信次数"] == 0
    # 被 em 就近匹配佐证的批次天然自洽(双源一致),不因 em 口径上限被误标
    matched = [u for u in payload["解禁"] if u.get("增强源") == "em"]
    assert matched and all(u["自洽性"] == "可采信" for u in matched)


def test_em匹配上的解禁批次不被自洽性护栏误伤():
    """⑤ 就近匹配上 em 的批次 = 双源一致,即便其数量恰等于 em 口径上限也不判不可采信。"""
    sina = [{"解禁日期": "2026-12-01", "解禁数量": 200.0, "公告日期": "2026-04-29"}]  # 200万股
    em = [{"解禁时间": "2026-12-01", "实际解禁数量": 2_000_000, "未解禁数量": 0,
           "占流通市值比例": 5.0}]
    rows, stat = efin.normalize_unlocks(sina, em)
    assert rows[0]["增强源"] == "em"
    assert rows[0]["自洽性"] == "可采信"       # 匹配上 → 跳过上限核验
    assert stat["不可采信"] == 0
