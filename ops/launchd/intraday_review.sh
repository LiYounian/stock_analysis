#!/bin/bash
# 午盘选股复盘(供 launchd 调用):工作日 17:00 触发,对当日午盘选出的票记「下午 α」。
#   读午盘节点落盘的 11:30 快照 + 主档当日收盘 → 下午涨跌%/下午等权基准 → 买入组/规避组逐票 α
#   → 产 docs/每日分析/复盘/午盘_<date>.md + 追加结论到经验 inbox。**纯确定性、不跑 LLM、不触网。**
#
# 时点(D9 方案a):17:00 定在 pull_refresh(15:40 起、全A自采~73min→主档当日全A收盘约 16:50 齐)之后,
#   留 ~10min 余量读主档收盘。若某日 pull_refresh 跑超时、主档当日收盘未就绪,复盘会**优雅降级**
#   (基准 NaN、票记「—」),不崩;届时把本 job 往后挪即可。
